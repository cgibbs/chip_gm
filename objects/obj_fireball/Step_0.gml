...
if (global.ticks % 10 = 0) {
	// do fireball stuff once every n ticks

	// if hit wall, turn right, left, or back
	if (!check_forward_empty(self.facing)) {
		if (!check_relative_right_wall_exists(self.facing)) {
			self.facing = return_relative_right(self.facing);
			move_forward(self.facing);
		} else if (!check_relative_left_wall_exists(self.facing)) {
